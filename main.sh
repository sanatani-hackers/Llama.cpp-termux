...
INSTALL_DIR="$PREFIX/share/bitnet"
...
# Safety blocks for disabled options (Updated for new numbers 2-5)
if [ "$choice" == "3" ] && [ "$GPU_SUPPORTED" = false ]; then
    echo -e "${R}[!] GPU Build is disabled for your hardware ($GPU_TYPE).${N}"
    echo -e "${Y}Falling back to CPU-only build...${N}"
    choice=2
fi

if [ "$choice" == "4" ] && [ "$TURNIP_SUPPORTED" = false ]; then
...
# ==============================================================================
# DEPENDENCIES & CLONING (Updated for choices 1-5)
# ==============================================================================
...
if [[ "$choice" =~ ^[1-5]$ ]]; then
    ...
    if [ ! -d "qvac-fabric-llm.cpp" ] && [[ "$choice" =~ ^[2-5]$ ]]; then
    ...
    elif [ -d "qvac-fabric-llm.cpp" ] && [[ "$choice" =~ ^[2-5]$ ]]; then
        cd qvac-fabric-llm.cpp || exit
        export ASAN_OPTIONS=detect_leaks=0
    fi
fi
...
# ==============================================================================
# COMPILATION LOGIC AND PRE-BUILT BINARY DOWNLOAD (ENHANCED)
# ==============================================================================
...
if [ "$choice" == "1" ]; then
    ...
    echo -e "${Y}[*] Installing GPU Vulkan Stack...${N}"
    apt install vulkan-headers vulkan-loader-generic shaderc -y >/dev/null 2>&1
    ...
    echo -e "${Y}[*] Installing Turnip Open-Source Drivers...${N}"
    apt install vulkan-headers mesa-vulkan-icd-freedreno vulkan-loader-generic shaderc -y >/dev/null 2>&1
...
if [[ "$choice" =~ ^[2-5]$ ]]; then
    check_error "Compilation Phase"
fi

# ==============================================================================
# GLOBAL INSTALLATION & PATH INJECTION
# ==============================================================================
if [[ "$choice" =~ ^[1-5]$ ]]; then
    echo -e "\n${Y}[*] Finalizing Global Installation to $INSTALL_DIR...${N}"
    mkdir -p "$INSTALL_DIR/models" "$PREFIX/bin"

    # Only copy files IF we compiled from source (Options 2, 3, 4, 5)
    # If choice is 1 (Pre-built), the tarball already extracted them!
    if [[ "$choice" =~ ^[2-5]$ ]]; then
        cp bin/llama-cli "$INSTALL_DIR/" 2>/dev/null || cp llama-cli "$INSTALL_DIR/" 2>/dev/null
        cp bin/libllama.so* "$INSTALL_DIR/" 2>/dev/null || cp libllama.so* "$INSTALL_DIR/" 2>/dev/null
        cp bin/llama-server "$INSTALL_DIR/" 2>/dev/null || cp llama-server "$INSTALL_DIR/" 2>/dev/null
        cd ..
    fi
    ...
    # Smart wrapper assignment based on new numbers
    # Option 4 is Turnip. Option 1 dynamically checks if Turnip is supported.
    if [ "$choice" == "4" ] || ([ "$choice" == "1" ] && [ "$TURNIP_SUPPORTED" = true ]); then
        ...
        echo -e "${G}[+] Turnip GPU enabled globally!${N}"
        
    # Option 3 is Standard Vulkan.
    elif [ "$choice" == "3" ] || ([ "$choice" == "1" ] && [ "$GPU_SUPPORTED" = true ]); then
        echo "$INSTALL_DIR/llama-cli -ngl 99 -t $REC_THREADS \"\$@\"" >> $WRAPPER
        echo -e "${G}[+] Standard GPU acceleration enabled globally${N}"
        
    # Option 2 (CPU) or unsupported hardware fallback
    else
        echo "$INSTALL_DIR/llama-cli -t $REC_THREADS \"\$@\"" >> $WRAPPER
        echo -e "${G}[+] CPU-only mode activated globally${N}"
    fi

    chmod +x $WRAPPER
    echo -e "\n${G}[✓] INSTALLATION SUCCESSFUL!${N}"
...
if [ "$GPU_SUPPORTED" = true ] && [[ "$choice" =~ ^[134]$ ]]; then
